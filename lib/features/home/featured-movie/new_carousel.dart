...
//       descendantsAreFocusable: false, // Important: prevent PageView from getting focus
//       onKey: (FocusNode node, RawKeyEvent event) {
//         if (event is RawKeyDownEvent) {
//           if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
//             // When down arrow is pressed, find the next focusable node below
//             final currentContext = node.context;
//             if (currentContext != null && isTV) {
//               // Move focus to the next focusable node
//               final nextFocus = node.nextFocus();
              
//               // If we couldn't move focus forward normally, try explicitly finding
//               // the next focusable element below
//               if (!nextFocus) {
//                 print("Default focus traversal couldn't find next focus");
//                 // Try to find any focusable widget below the carousel
//                 FocusScope.of(context).focusInDirection(TraversalDirection.down);
//               }
              
//               // Signal that we've handled this key
//               return KeyEventResult.handled;
//             }
//             return KeyEventResult.ignored; // Non-TV platform, pass it through
...
class _SimpleFeaturedCarouselState extends State<SimpleFeaturedCarousel> {
  ...
  // Find the first focusable widget in the section below
  FocusNode? _findFirstFocusNodeInNext() {
    // First, try to find a film card or history card focus node
    FocusNode? targetNode;
    
    FocusManager.instance.rootScope.descendants.forEach((node) {
      final String? label = node.debugLabel;
      if (label != null) {
        if (label.startsWith('film_card_') || 
            label.startsWith('history_card_') ||
            label.startsWith('fav_card_')) {
          if (targetNode == null) {
            targetNode = node;
          }
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Focus(
      ...
      onKey: (FocusNode node, RawKeyEvent event) {
        if (event is RawKeyDownEvent) {
          if (event.logicalKey == LogicalKeyboardKey.arrowDown) {
            print("Down arrow pressed in featured carousel");
            
            // Try to find the first focusable node in the next section
            final firstNodeInNext = _findFirstFocusNodeInNext();
            if (firstNodeInNext != null) {
              print("Found next focusable node: ${firstNodeInNext.debugLabel}");
              firstNodeInNext.requestFocus();
              return KeyEventResult.handled;
            }
            
            // If no film card found, allow default focus traversal
            print("No film cards found, using default focus traversal");
            return KeyEventResult.ignored;
          } else if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
            if (_currentIndex < movies.length - 1) {
              _pageController.nextPage(
